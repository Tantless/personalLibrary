...
* 新增 M3C expanded eval set，目标是暴露问题而不是马上强制全部通过。
...
* [ ] M3C 设计明确 eval schema v2 的新增字段和向后兼容策略。
* [ ] M3C 设计明确 locked regression set 与 expanded diagnostic set 的区别。
* [ ] M3C 设计明确 simple vs planned 的比较报告字段。
* [ ] M3C 设计明确 pass-level diagnostics 和失败分类。
* [ ] M3C 设计明确哪些结果进入 CI gate，哪些只进入本地报告。
* [ ] M3C 设计明确 semantic/hybrid retrieval 的进入条件，不在本任务直接实现 pgvector/reranker。
* [ ] 用户确认 M3C MVP 范围与 PR 顺序。
...
* Add expanded diagnostic queries in a separate fixture or separate suite field.
...
* Requires careful fixture curation.
...
* Expanded diagnostic query set, initially report-only.
...
* Expanded diagnostic queries may intentionally fail; they should not all become CI blockers on day one.
...
### PR3: Expanded diagnostic query set

* Add non-private diagnostic queries covering `zh`、`en`、`ja`、`mixed` where feasible.
* Keep expanded set report-only at first.
* Add locked regression threshold for current 6: planned top_10 and context support must remain 1.0.
...
**Decision**: M3C should first expand eval coverage and add planned-vs-simple diagnostics. Semantic/hybrid retrieval remains deferred until the diagnostic suite shows repeated lexical failure classes that cannot be addressed by source aliases, glossary, or evidence selection changes.

**Consequences**: M3C may not immediately improve user-facing recall, but it creates the measurement layer needed to make future retrieval changes defensible.

## Open Questions

* MVP scope preference: should M3C stop at evaluator/report design, or include the first expanded diagnostic query set in the same task?
