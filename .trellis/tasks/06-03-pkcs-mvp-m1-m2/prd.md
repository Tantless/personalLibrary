...
* [ ] `knowledge_type` filter 支持 `ai_conversation` 与 `document`。
...
* [ ] citation 同时包含 `line_start` / `line_end` 和 knowledge_type-specific metadata。
* [ ] `ingest_source` 支持显式 `canonical_key`，未传时按 knowledge_type 前缀 + 五位数据库递增编号生成。
* [ ] 删除导入前原文件后，`read_source` 仍能从 Raw Archive 读回已摄入版本的证据。
* [ ] `data/raw/` 中真实资料不会被 git 跟踪。
* [ ] 重复导入相同内容不会产生脏重复。
* [ ] 同一资料的新版本不会覆盖旧版本。
* [ ] `search_knowledge` 能在 3 秒内返回 top_k 搜索结果，并包含 source refs 与 snippets。
* [ ] `search_knowledge` 结果结构包含 result/chunk/source/version refs、canonical_key、title、source_format、normalized_format、knowledge_type、score、citation 和 metadata。
