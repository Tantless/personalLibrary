...
* 建立 Raw Archive，默认路径为项目内 `data/raw/`，按 knowledge_type 分层；原始资料追加保存，不因新版本覆盖旧版本。
...
* canonical source identity 使用 `canonical_key`；未传时自动推断为 `knowledge_type + ":" + normalized_absolute_file_path`。
...
* [ ] `ingest_source` 支持显式 `canonical_key`，未传时按 knowledge_type + normalized absolute path 推断。
...
* Canonical source identity behavior using explicit `canonical_key` or `knowledge_type:absolute_path` fallback.
...
* Confirmed canonical source identity: explicit `canonical_key`, with knowledge_type + normalized absolute path fallback.
