...
* 建立 Raw Archive，默认路径为项目内 `data/raw/`，按 knowledge_type 分层；原始资料追加保存，不因新版本覆盖旧版本，并作为 ingest 后系统内部认定的源文件。
...
* 建立 PostgreSQL 主数据结构，至少包含 `sources`、`source_versions`、`chunks`、`citations`、`ingest_jobs`、`source_key_counters`。
* MVP knowledge_type 枚举包含 `ai_conversation` 与 `document`。
...
* 支持 Markdown/Web 文档 ingest，输入格式只包含本地 `.md` / `.txt` 文件；至少保存 title、heading_path、section locator、content。
* chunk 策略采用结构优先 + 上限切分：Markdown/文本按 heading 或 section，AI 对话按 turn 或相邻 turns window，超长再切分并保留少量 overlap。
* locator 采用两层设计：主 locator 为统一行号范围；metadata 保留 heading_path、turn/window、roles 等 knowledge_type-specific 语义信息。
* canonical source identity 使用 `canonical_key`；未传时按 knowledge_type 前缀 + 五位数据库递增编号生成，例如 `D00001`、`A00001`。
* 用户导入时的完整原始路径只视为单次输入位置，不作为长期 source identity 或 version identity 入库。
* 支持重复导入检测：相同内容 hash 不产生脏重复；同一 canonical source 的新 hash 产生新 version。
* 支持 PostgreSQL full-text search，支持 query、knowledge_type filter、canonical_key filter、top_k；排序采用 FTS rank + title boost。
* `search_knowledge` 输出稳定证据结构，包含 `result_id/chunk_id`、`source_id`、`version_id`、`canonical_key`、`title`、`source_format`、`normalized_format`、`knowledge_type`、`snippet`、`score`、`citation`、`metadata`。
...
* [x] 可以启动 PKCS 本地服务，并通过 HTTP `health_check` 和 MCP `health_check` 验证服务可用。
* [x] 可以通过 CLI 执行 ingest/search/read/context-pack 基础流程。
* [x] 可以通过 Docker Compose 启动 PostgreSQL，服务重启后数据库数据不丢。
* [x] migration 创建 canonical_key/content_hash/source-version 相关 schema indexes 与 FTS GIN index。
* [x] `search_vector` 由数据库层自动生成，ingest 代码不直接写入该字段。
* [x] FTS 查询与 search_vector 使用 `simple` 配置。
* [x] 可以运行 pytest 单元测试和连接 Docker Compose PostgreSQL 的集成测试。
* [x] 可以导入至少 10 条 AI 对话样例和 10 篇 Markdown/Web 文档样例。
* [x] `knowledge_type` filter 支持 `ai_conversation` 与 `document`。
* [x] `search_knowledge` 支持 `canonical_key` filter，用于限定某一长期 source 内搜索。
* [x] AI 对话样例覆盖 Markdown/transcript 与 JSONL 两种格式。
* [x] Markdown/Web 文档样例只要求覆盖 `.md` / `.txt` 文件。
* [x] `ingest_source` 接收本地文件路径并拒绝/不暴露 raw content 与 URL 抓取输入。
* [x] `ingest_source` 可以导入单个文件，也可以导入目录下当前层级的匹配文件；不递归进入子目录。
* [x] 目录导入中单个文件失败不会阻止其他文件处理，report 包含 succeeded/skipped/failed。
* [x] 单文件和目录导入都返回同一 ingest report shape。
* [x] 导入后 Raw Archive 中存在原始资料文件，数据库中存在 source、version、chunk、citation 记录。
* [x] chunk metadata 保留 Markdown heading_path 或 AI conversation turn/window 信息。
* [x] citation 同时包含 `line_start` / `line_end` 和 knowledge_type-specific metadata。
* [x] `ingest_source` 支持显式 `canonical_key`，未传时按知识类型前缀 + 五位递增编号生成。
* [x] 删除导入前原文件后，`read_source` 仍能从 Raw Archive 读回已摄入版本的证据。
* [x] `data/raw/` 中真实资料不会被 git 跟踪。
* [x] 重复导入相同内容不会产生脏重复。
* [x] 同一资料的新版本不会覆盖旧版本。
* [x] `search_knowledge` 能在 3 秒内返回 top_k 搜索结果，并包含 source refs 与 snippets。
* [x] `search_knowledge` 结果结构包含 result/chunk/source/version refs、canonical_key、title、source_format、normalized_format、knowledge_type、score、citation 和 metadata。
* [x] 标题命中的搜索结果相对纯正文命中有 boost，默认不使用 recency boost。
* [x] `read_source` 能在 1 秒内按 citation 读回正确原文片段。
* [x] `read_source` 支持 `context_lines`，并默认不返回整份 source。
* [x] `read_source` 支持 `chunk_id` addressing 和 `source_id/version_id/locator` addressing。
* [x] `get_context_pack` 返回外层 JSON，且包含 `query`、`retrieval_plan`、`sources`、`followup_read_suggestions`、`context_pack_markdown`。
* [x] `context_pack_markdown` 包含 `Conflicts / Caveats` 段落，并明确 MVP 未做真正冲突检测。
* [x] Context Pack 不会被单一 source 的相邻 chunks 占满。
* [x] Context Pack 默认最多包含 10 条 evidence，且每个 source 最多 3 条。
* [x] `budget_tokens` 可以影响 `context_pack_markdown` 长度，但文档明确它是软限制而非精确 token guarantee。
* [x] Context Pack 中每个 evidence 都能映射到可读回的 source/version/locator。
* [x] 构造至少 20 个 MVP 检索问题，目标资料进入 top 10 的比例不低于 80%，进入 top 5 的比例不低于 60%。
* [x] 检索问题存放在 `tests/fixtures/eval_queries.jsonl`，每行包含 query expectation。
* [x] `tests/fixtures/` 中不包含真实个人私密资料。
* [x] Codex 优先调用 MCP tools 完成一次“检索过去资料并生成上下文”的任务；若 Codex MCP 配置受限，则用任意 MCP client smoke test fallback。
...
* PR4: PostgreSQL FTS SearchProvider, `search_knowledge`, knowledge_type filter, title boost, top_k behavior.
...
* PR4 verifies PostgreSQL FTS `simple` search, database-generated `search_vector`, title boost, knowledge_type/canonical_key filters, top_k, stable search result shape, and latency target.
...
* `RawArchiveWriter` writing under knowledge_type/source_id/version_id.
...
* Markdown/text parser for `document` with heading_path metadata and line locators.
...
* Canonical source identity behavior using explicit `canonical_key` or knowledge-type prefix plus five-digit database counter fallback.
...
* Raw Archive writes under `knowledge_type/source_id/version_id`.
...
* `knowledge_type`, `canonical_key`, and `top_k` filters.
* Stable search response shape with result/chunk/source/version refs, canonical key, title, source format, normalized format, knowledge type, snippet, score, citation, and metadata.
...
* Eval query fields: `query`, `expected_fixture`, `expected_canonical_keys`, `expected_knowledge_types`, and `notes`.
...
* Confirmed locator format: unified line range plus knowledge_type-specific metadata.
* Confirmed canonical source identity: explicit `canonical_key`, with knowledge-type prefix plus five-digit database counter fallback.
* Confirmed ingest path identity: original full input paths are one-time consumption paths only; Raw Archive is the internal source file after ingest.
* Confirmed knowledge_type names: `ai_conversation`, `document`.
...
* Confirmed search filters: `knowledge_type`, `canonical_key`, and `top_k`; no title/date filters in MVP.
